...
1. **What is the precise session-scope definition of `errorsInARow` for the confidenceScore formula?**
...
2. **Does the session-end combined screen replace `main.ts`'s bare "Урок завершён!" for BOTH the main-pass-complete-with-non-empty-reviewQueue transition AND the fully-complete (reviewQueue empty) state, or only the latter?**
