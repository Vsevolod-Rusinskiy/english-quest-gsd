...
stopped_at: "Uncapped theory 'Не понятно' (endless fresh LLM re-explanations, commit 04a5017), after the 2026-07-18 fast-mode UX/design batch (two-section theory, per-exercise RU translations, local Ollama backend, unified design system)"
...
- [UX finding, 2026-07-06, RESOLVED 2026-07-18 fast-mode] ~~Theory screen "Не понятно" 3rd-tap transition feels abrupt: after `maxSimplifyRounds` (3) the soft transition auto-advances the child straight to the first exercise even though they just tapped "Не понятно" again, with no explanatory cue.~~ Resolved by user decision to REMOVE the cap entirely: `handleTheoryStep` no longer consults `maxSimplifyRounds` and never sets `theoryUnderstood` on a "не понятно" tap — the child can tap indefinitely and always gets a NEW LLM-generated variant (only an explicit "Понятно" advances to practice). Variety is real, not repeated text: the caller threads the on-screen explanation back in as `currentLevelText`, the Theory Tutor prompt demands a fresh angle + new example each round, and the call uses `temperature: 0.8` (new optional per-call temperature plumbed through `callAgent`/`ollamaClient`; other agents stay deterministic). Verified live against qwen3.5:4b: 4 consecutive rounds produced 4 distinct analogies (toy in a closet → apples in a shop → kitchen clock → sibling in the bath). `theory.maxSimplifyRounds` in `Lesson-1A.json` is now unused (field kept for schema compatibility).
...
| 17 | Uncap theory "Не понятно": remove maxSimplifyRounds soft-transition, generate a NEW LLM variant every round indefinitely (thread previous explanation + per-call temperature 0.8); only "Понятно" advances | 2026-07-18 | 04a5017 | — |
