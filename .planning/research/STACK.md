...
**Domain:** Browser-only, no-backend interactive learning app (deterministic core + 5 single-shot LLM "agent function" calls)
**Researched:** 2026-07-01
**Confidence:** MEDIUM-HIGH (core library versions cross-checked across npm/official docs; Claude API structured-output details verified against official Claude Platform docs and multiple independent write-ups)

## Framing: this is not a typical "pick a framework" decision

The project's own constraints already answer most of the usual stack questions:
...
The two real technical decisions are: **(1) what to build the UI/state in**, and **(2) how to call Claude from a browser-only app without leaking the API key.** Everything else is supporting tooling.
...
# Dev dependencies
npm install -D typescript vite vitest eslint typescript-eslint prettier
...
**If you want the simplest possible "no backend at all" demo (grader supplies key):**
...
**If you want a smooth one-click demo for defense/presentation:**
...
**If component structure feels valuable for the UI (theory screen, 4 exercise-type renderers, review queue, parent report):**
...
---
*Stack research for: English Quest (browser-only, no-backend English-learning MVP with deterministic core + 5 single-shot Claude "agent function" calls)*
*Researched: 2026-07-01*
